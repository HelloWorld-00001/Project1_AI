...
class AlphaBeta:
    ...
    def limit_alpha_beta(self, node, LIMIT_TIME):
        ...
        best = None
        if (node.size <= 3): # if size of tic tac toe < 3 -> no limit
            best = self.alpha_beta_search(node, "O", 0, 10000000)
        else:
            while True:
...
class TicTacToe:
    ...
    def random_child(self):
        x = rd.randint(0, self.size)
        y = rd.randint(0, self.size)
        while self.is_valid(x, y) == 0:
            x = rd.randint(0, self.size)
            y = rd.randint(0, self.size)
